...
function PipBTN({ isMobile, isTab }: CommonBtnProps) {
  ...
  const togglePipMode = async () => {
    ...
    if ("pictureInPictureEnabled" in document) {
      ...
      pipVideo.addEventListener("leavepictureinpicture", (event) => {
        pipWindowRef.current = null;
        setPipMode(false);
        const srcObject = pipVideo.srcObject;
        if (srcObject && "getTracks" in srcObject) {
          (srcObject as MediaStream).getTracks().forEach((track) => track.stop());
        }
      });
...
export function BottomBar({
  ...
  const RaiseHandBTN = ({ isMobile, isTab }: CommonBtnProps) => {
  ...
  const WebCamBTN = () => {
    const mMeeting = useMeeting();
    // const { selectWebcamDeviceId } = useMeetingAppContext(); // Removed to use prop from closure
          ...
          onClick={async () => {
            ...
            if (!localWebcamOn) {
              track = await getVideoTrack({
                webcamId: selectWebcamDeviceId || undefined,
  ...
  const ScreenShareBTN = ({ isMobile, isTab }: CommonBtnProps) => {
  ...
  const ChatBTN = ({ isMobile, isTab }: CommonBtnProps) => {
    return isMobile || isTab ? (
      <MobileIconButton
        tooltipTitle={"Chat"}
        buttonText={"Chat"}
  ...
  const ParticipantsBTN = ({ isMobile, isTab }: CommonBtnProps) => {
  ...
  const MeetingIdCopyBTN = (props?: Partial<CommonBtnProps>) => {
