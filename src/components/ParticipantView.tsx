...
export const CornerDisplayName = ({
  ...
  const [coords, setCoords] = useState<{ top: number; left: number } | null>(null); // takes current button coordinates
    ...
    },
    {
      ...
      audio:
        audioStats && audioStats[0]?.jitter
          ? `${parseFloat(String(audioStats[0]?.jitter)).toFixed(2)} ms`
          : "-",
      video:
        videoStats && videoStats[0]?.jitter
          ? `${parseFloat(String(videoStats[0]?.jitter)).toFixed(2)} ms`
          : "-",
    },
    {
      label: "Packet Loss",
